...
public class VarInstSplit implements SLAlg {
	private static final int MAX_NB_CLUSTERS = 3000;//turned it to 3000...from 2000
	...
	public GraphSPN learnStructure(Dataset d) {
		...
		while(!toProcess.isEmpty()){
			...
			if(currentSlice.variables.length == 1){
				Node varnode = new SmoothedMultinomialNode(d, currentSlice.variables[0], currentSlice.instances);
                                //Node varnode = new PolynomialNode(d, currentSlice.variables[0], currentSlice.instances_;
        ...
        public GraphSPN learnPolyStructure(Dataset d) {
		this.d = d;
                
		int numCmsg = 6;
                
                //PolynomialNode node =  new PolynomialNode(this.d,0,int[]{0,1,10}); 
		...
		while(!toProcess.isEmpty()){
			...
			if(currentSlice.variables.length == 1){
				//Node varnode = new SmoothedMultinomialNode(d, currentSlice.variables[0], currentSlice.instances);
                                if(this.d.getVariableType()[currentSlice.variables[0]] == 0){ //case where its one-hot encoded
                                    //System.out.println("var 0 type: " + this.d.getVariableType()[currentSlice.variables[0]]);
                                    Node varnode = new SmoothedMultinomialNode(d, currentSlice.variables[0], currentSlice.instances);
                                    spn.order.add(varnode);
                                    idNodeMap.put(id, varnode);
                                }
                                else{
                                    //System.out.println("var 1 type: " + this.d.getVariableType()[currentSlice.variables[0]]);
                                    Node varnode = new PolynomialNode(d, currentSlice.variables[0], currentSlice.instances);
                                    spn.order.add(varnode);
                                    idNodeMap.put(id, varnode);
                                }
				continue;
			...
			for(int it=0; it<numEMits; it++){
				...
				for(int inst : instanceOrder){
					...
					if(bestCluster == null){
						...
						if(nbcs.size() > MAX_NB_CLUSTERS && currentSlice.instances.length > 100000){ //added a zero to 10000
